...
// PUBLIC ENTRY POINT
// ==============================================================
const GE_WEB_APP_VERSION = '2026.07.27.2';
const GE_SUPPORTED_ACTIONS = [
  'ping',
  'getData',
  'checkName',
  'mobileBridgePing',
...
function doGet(e) {
  ...
  const rawAction = String(params.action || 'getData');
  const action = normalizeGraduateAction_(rawAction);
    ...
    if (action === 'ping' || action === 'mobileBridgePing') {
      ...
      payload = {
        ...
        endpoints: {
          ...
          getData: 'getData',
          checkName: 'checkName',
          mobileBridgePing: 'mobileBridgePing',
          mobileBridgeCheckName: 'mobileBridgeCheckName'
        }
      };
    } else {
      payload = {
        ...
        version: GE_WEB_APP_VERSION,
        action: rawAction,
        supportedActions: GE_SUPPORTED_ACTIONS,
        message: 'Unsupported action: ' + rawAction + '. Update the Web App deployment to the latest code.gs or use action=getData.'
      };
    }
  } catch (err) {
    payload = {
  ...
  if (payload && typeof payload === 'object') {
    if (!payload.service) payload.service = 'graduateEmployability';
    if (!payload.version) payload.version = GE_WEB_APP_VERSION;
  }
...
/**
 * Optional POST compatibility. The Yearbook Quest front end currently uses
 * GET/JSONP, but accepting POST makes the backend safer for future clients.
 */
function doPost(e) {
  var params = {};
  if (e && e.parameter) {
  ...
  try {
    ...
    // Non-JSON POST bodies continue using e.parameter.
  }

  return doGet({ parameter: params });
}

function normalizeGraduateAction_(value) {
  var key = String(value || '')
    .toLowerCase()
    .replace(/[^a-z0-9]/g, '');

  if (!key || key === 'getdata' || key === 'graduatemployability' || key === 'listresponses') return 'getData';
  if (key === 'ping' || key === 'health' || key === 'status') return 'ping';
  if (key === 'mobilebridgeping') return 'mobileBridgePing';
  if (
    key === 'checkname' ||
    key === 'verifyname' ||
    key === 'findname' ||
    key === 'lookupname' ||
    key === 'checkgraduate' ||
    key === 'checkemployability'
  ) return 'checkName';
  if (key === 'mobilebridgecheckname') return 'mobileBridgeCheckName';
