...
function loadSettings() {
  try {
    ...
    const legacy = localStorage.getItem('yearbookQuestDataSourcesV3');
...
async function testAppsScriptConnection({ inputId, settingKey, statusId, service }) {
  ...
  try {
    const payload = await jsonpRequest(inspected.url, { action: 'ping', service }, 30000);
    if (!payload?.success) throw new Error(payload?.message || 'The endpoint responded with an error.');
    setEndpointConnectionStatus(statusId, 'Connected successfully on this device ✓', 'success');
    return true;
  } catch (error) {
