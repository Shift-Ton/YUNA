...
function doGet(e) {
  ...
  const action = String(params.action || 'setupSheets');
  const callback = params.callback;
  let payload;

  try {
    // Ensure that the required sheets and columns exist before every request.
    ensureYearbookSheets_();

    if (action === 'ping') {
      payload = { success: true, service: 'paidList', timestamp: new Date().toISOString() };
